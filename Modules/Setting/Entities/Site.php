...
class Site extends BaseModel implements HasMedia
{
    ...
    public function getLogoStickyAttribute()
    {
        $media = $this->getMedia(config('core.media_collection.setting_site.logo_sticky'));
        if (isset($media[0])) {
            return json_decode(json_encode([
                'file_name' => $media[0]->file_name,
                'file_url' => $media[0]->getUrl()
    ...
    public function uploadFiles()
    {
        ...
        // check for logo sticky
        if (request()->hasFile('logo_sticky')) {
            // remove old file from collection
            if ($this->hasMedia(config('core.media_collection.setting_site.logo_sticky'))) {
                $this->clearMediaCollection(config('core.media_collection.setting_site.logo_sticky'));
            }
            // upload new file to collection
            $this->addMediaFromRequest('logo_sticky')
                ->toMediaCollection(config('core.media_collection.setting_site.logo_sticky'));
        }
